# Proposal to include Nostromo Smart Contract

## Proposal
Allow Nostromo Smart Contract to be deployed on Qubic. 
...
## What is Nostromo
Nostromo is the first sci-fi-inspired launchpad built on the Qubic blockchain, serving as a decentralized platform for connecting innovative projects with investors through secure, community-driven Initial DEX Offerings (IDOs). It aims to facilitate token sales and fundraising in a transparent, efficient manner, empowering developers to launch projects while fostering a vibrant ecosystem for decentralized finance and blockchain innovation on Qubic.

## Nostromo Key Features
1. **Community-Driven IDOs:** Enables projects to conduct token sales with community approval, ensuring alignment with stakeholder interests and reducing development overlap through proactive mechanisms like Requests for Proposals (RFPs).
2. **Secure Fundraising:** Leverages Qubic's robust infrastructure for safe, peer-reviewed launches, with built-in staking options to reward participants and enhance network security.
3. **Sci-Fi Themed Web3 Platform:** Offers an engaging, futuristic interface for users to participate in launches, staking, and governance, making blockchain accessible and inclusive.
4. **Transparency and Scalability:** Features rigorous audits, on-chain voting tools, and integration with Qubic's high TPS for seamless, low-cost operations, supporting tools like Metamask integration for user-friendly access.

## Fee Income Distribution
>10% Team Fee: Fixed 10% cut from total epoch revenue before any other distribution, ensuring team funding from all collected fees.

>90% for share holders

## Technical Implementation
From: https://github.com/qubic/core/pull/437/
