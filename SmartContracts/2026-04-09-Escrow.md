# Proposal to include Escrow Smart Contract

## Proposal
Allow Escrow Smart Contract to be deployed on Qubic.
...
# Qubic OTC Escrow Smart Contract

---

## 🚀 Introduction

The Qubic Escrow Smart Contract represents a solution for decentralized over-the-counter trading within the Qubic ecosystem. This project introduces a trustless, atomic swap mechanism that enables secure multi-asset trading between tokens, shares, and qu without requiring trusted third parties.

Our smart contract addresses critical limitations in the current Qubic trading infrastructure by enabling sub-qu token pricing, multi-asset bundling, and secure peer-to-peer transactions. By implementing a decentralized escrow system, we eliminate counterparty risk while providing unprecedented flexibility in deal structuring.

The project aims to become the cornerstone of OTC trading in the Qubic ecosystem, fostering liquidity, price discovery, and reducing fraudulent activities that currently plague manual OTC transactions.
...
**1. Scam Prevention in OTC Trading**
The current OTC trading landscape in Qubic suffers from significant trust issues. Without a decentralized escrow mechanism, traders must rely on reputation systems or trusted intermediaries, leading to:
- High risk of fraud and scams
- Limited trading opportunities due to trust concerns
- Inefficient price discovery mechanisms
- Barrier to entry for new market participants

**2. Complex Multi-Asset Trading**
Current trading methods lack support for:
- Simultaneous multi-token exchanges
- Token-to-token direct swaps
- Complex deal structures combining different asset types
- Efficient bundling of assets in single transactions

### Market Opportunity

The Qubic ecosystem is experiencing rapid growth with increasing token diversity and trading volume. Our research indicates:
- Growing demand for secure OTC trading solutions
- Increasing number of token projects requiring flexible pricing mechanisms
- Community requests for trustless trading infrastructure
...
**Transaction Fees:**
- Deal creation fee: **250,000 qus** for posting a new deal (**non-refundable**)
- Deal fulfillment fee: **no base fee** on accept
- Volume-based commission on accept: **1% of qu** and **1% of tokens** involved in the deal
- Share fee on create and accept: **1,500,000 qus per each share** in the deal, charged from each side


**Revenue Distribution:**
- **qu revenue:** **93%** to shareholders, **2%** burn, **5%** to admins
- **token revenue:** **95%** to shareholders, **5%** to admins

**Deal Lifetime and Expiration:**
- Deal lifetime is **4 epochs** from creation.
- After expiration, the deal is removed automatically.
- Funds and share-fee amounts reserved for that deal are returned.
- The base creation fee (**250,000 qus**) is **not returned**.
...
**🔄 Multi-Asset Support**
- Support for up to 4 tokens/shares per side in a single deal
- Flexible asset combination possibilities
- Atomic swap guarantee for all supported assets

**🎯 Deal Types**
- **Open Deals**: Available for fulfillment by any Qubic address
- **Targeted Deals**: Restricted to specific recipient addresses

**⏳ Time-Bound Validity**
- Automatic expiration after 4 epochs
- Spam prevention through time limits
- Fresh deal guarantee for all participants
...
The smart contract supports a wide variety of OTC trade combinations between Qubic tokens, shares, and qu (native currency). Examples include:

- **Token → qu**  
  Sell any token for a specified amount of qu.
- **Token + Token + Token → qu**  
  Bundle multiple tokens into a single deal for qu payment.
- **Share + Share → qu**  
  Trade shares directly for qu.
- **Share → Share + qu**  
  Swap shares for other shares and receive additional qu.
- **Token ↔ Token**, **Token ↔ Share**, **Share ↔ Share**, **Token + Share ↔ Token + Share**, etc.

*Note: All tokens issued on QX can be used in deals.*
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Deal Creator  │    │  Smart Contract │    │  Deal Acceptor  │
│                 │    │     (Escrow)    │    │                 │
└─────────────────┘    └─────────────────┘    └─────────────────┘
         │                      │                       │
         │ 1. Create Deal       │                       │
         ...
         │ (Receive Assets)     │ (Execute Exchange)    │ (Receive Assets)
         │◄─────────────────────│─────────────────────▶│
         │                      │                       │
         │ 6. Deal Complete     │ 6. Deal Complete      │
         │ (Remove from List)   │ (Remove from List)    │
         │◄─────────────────────│─────────────────────▶│
```

## Technical Implementation
From: [qubic/core#821](https://github.com/qubic/core/pull/821)
