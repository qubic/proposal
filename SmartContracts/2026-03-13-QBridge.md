# Proposal to include QBridge Smart Contract (v2 - Multisig)

## Proposal
Allow the updated QBridge Smart Contract with multisig governance to be deployed on Qubic.
...
## What is QBridge
QBridge is a non-custodial cross-chain bridge connecting the Qubic blockchain with Ethereum.

It allows anyone to transfer QUBIC tokens between Qubic and ETH in seconds, leveraging a **2-of-3 multisig governance model** to ensure that no single party can unilaterally control admin operations, fee withdrawals, or manager changes.
...
| **Feature** | **Description** |
|-------------|-----------------|
| **2-of-3 Multisig Governance** | All critical admin operations (changing admins, adding/removing managers, withdrawing fees, changing thresholds) require approval from at least 2 out of 3 multisig admins via a proposal system. No single point of failure or trust. |
| **Proposal-Based Admin Actions** | Admin changes go through `createProposal` / `approveProposal` flow. Proposals can be cancelled by their creator. Once the threshold is reached, the action executes atomically. |
...
| **Fee Reservation System** | Fees from pending orders are reserved and not distributed until the order completes or is refunded. This prevents premature fee distribution for orders that may be refunded. |
| **Gas-Optimized Batch Relays** | Qubic side fees are negligible; on Ethereum the bridge bundles multiple proofs into a single transaction, amortizing gas and cutting user costs. |
| **Cross-Chain Message Bus** | Beyond tokens, QBridge carries arbitrary payloads (governance votes, oracle data, contract calls) for multi-chain dApps spanning Qubic's compute layer and Ethereum's DeFi stack. |
| **Instant Liquidity Pools** | For users who can't wait for finality, an LP-backed pool provides "liquidity vouchers" so assets arrive instantly, with LPs earning a small fee. |
| **Unified Wallet UX** | One-click bridge inside the Qubic wallet and popular ETH wallets; progress bar shows confirmations on both chains. |
| **Audited & Open-Source** | All contracts and validator code are open-source. The contract has been audited with all findings addressed (see Audit Fixes section below). |
| **Low-Fee Model** | The bridge charges a **1% total fee** (0.5% operator + 0.5% network) with a minimum order of 200 QU to prevent spam. |
| **Refund Protection** | All error paths refund tokens and fees to the user. Two-phase commit pattern ensures tokens are never lost. |

---

## Fee Model

The bridge charges a **1% total fee** on each order, split in two:

| **Fee** | **Rate** | **Destination** |
|---------|----------|-----------------|
| Operator fee | 0.5% | Distributed to Vottun via `feeRecipient` wallet |
| Network fee | 0.5% | Distributed to computors (shareholders) via `distributeDividends` |

- Fees are **reserved** when the order is created and only **released** for distribution when the order completes.
- On refund, fees are returned to the user (Qubic-to-ETH) or released without refund (ETH-to-Qubic, since the bridge operator paid them).
- **Minimum order amount:** 200 QU (ensures fees are at least 1 QU per direction, preventing zero-fee spam).

---

## Multisig Governance

The contract uses a **2-of-3 multisig** model for all administrative operations:

### Proposal Types
| **Type** | **Action** |
|----------|-----------|
| `SET_ADMIN` | Replace an existing multisig admin with a new address |
| `ADD_MANAGER` | Add a new operational manager (max 3) |
| `REMOVE_MANAGER` | Remove an operational manager |
| `WITHDRAW_FEES` | Withdraw accumulated operator fees |
| `CHANGE_THRESHOLD` | Change the required approvals threshold (min 2) |

### Flow
1. A multisig admin creates a proposal via `createProposal` (auto-counts as 1st approval)
2. A second multisig admin approves via `approveProposal`
3. Once threshold (2) is reached, the action executes atomically
4. If the action fails, the proposal is deactivated (admins can create a new one)
5. The creator can cancel a pending proposal via `cancelProposal`

### Roles
- **Multisig Admins (3):** govern the contract through proposals
- **Managers (max 3):** operational role that can `completeOrder`, `refundOrder`, and `addLiquidity`

---

## Audit Fixes Incorporated

The contract addresses all findings from the security audit:

| **ID** | **Fix** |
|--------|---------|
| QVB-02 | Single-pass slot allocation for orders (find empty OR recycle in one loop) |
| QVB-03 | Single-pass proposal slot finding with recycling |
| QVB-04 | Removed redundant `netAmount` variable |
| QVB-09 | Minimum order amount, liquidity reserved at `createOrder`, zero-fee prevention |
| QVB-10 | Refund tokens on all error paths (`transferToContract`, `addLiquidity`) |
| QVB-11 | Capture `invocationReward()` immediately, refund excess payment |
| QVB-12 | Transfer-first pattern: state updated only after successful transfer |
| QVB-13 | Check `transfer()` return value with `>= 0` (not truthy) |
| QVB-17 | `nextOrderId` incremented only on successful order creation |
| QVB-18 | Validate ETH address not all zeros, validate proposal addresses not `NULL_ID` |
| QVB-20 | Proposals marked as executed only if the underlying action succeeded |
| QVB-21 | Two-phase commit: `tokensReceived` set at `transferToContract`, `tokensLocked` set at `completeOrder` |
| QVB-22 | Reject EVM-to-Qubic orders in `transferToContract` (not applicable for that direction) |
...
|--------|-------------|-----------------|
| 1 | `getOrder` | Get order details by ID |
| 2 | `isManager` | Check if an address is a manager |
| 3 | `getTotalReceivedTokens` | Total tokens ever received |
| 4 | `getTotalLockedTokens` | Current locked token balance |
| 5 | `getOrderByDetails` | Find order by ETH address + amount + status |
| 6 | `getContractInfo` | Full contract state including multisig info |
| 7 | `getAvailableFees` | Available (non-reserved) fees |
| 8 | `getProposal` | Get proposal details by ID |

**Procedures (state-changing):**

| **ID** | **Procedure** | **Description** |
|--------|--------------|-----------------|
| 1 | `createOrder` | Create a bridge order (Qubic-to-ETH or ETH-to-Qubic) |
| 2 | `completeOrder` | Manager completes an order (releases/locks tokens) |
| 3 | `refundOrder` | Manager refunds an order (returns tokens + fees) |
| 4 | `transferToContract` | User deposits tokens for a Qubic-to-ETH order |
| 5 | `addLiquidity` | Manager/admin adds liquidity to the bridge |
| 6 | `createProposal` | Multisig admin creates an admin proposal |
| 7 | `approveProposal` | Multisig admin approves a pending proposal |
| 8 | `cancelProposal` | Proposal creator cancels a pending proposal |

### Source Code

See the PR: https://github.com/qubic/core/pull/786
