# Proposal: Upgrade Multi-Sign-Vault (MSVault) to version 2.0 with Multi-Asset Support

## Proposal

Upgrade the existing MSVault Smart Contract to include comprehensive support for custom assets (tokens). 
...
The original MSVault contract provided a secure, multi-signature solution for storing Qubics. This upgrade preserves all existing functionality while expanding its capabilities to allow vaults to securely store, manage, and transfer any custom asset created on the Qubic network.

This transforms MSVault from a simple Qubic wallet into an essential infrastructure component for decentralized projects, and individuals who need to secure tokenized assets, manage treasuries, or co-own digital property with the same high level of security.

## Key New Features
### 1. Multi-Asset Storage

Vaults are no longer limited to holding just Qubics. They can now store a diverse portfolio of custom assets. This is perfect for:
* Project treasuries holding their native tokens.
* Decentralized organizations managing a variety of assets.
* Individuals securing their collection of different tokens in one safe place.

### 2. Multi-Signature Asset Release

The core "Y of X" approval logic that secures Qubic transfers has been extended to custom assets. To send a token from a vault, the required number of owners must approve the transaction, providing the same protection against unauthorized access or single points of failure.

### 3. Asset Management Rights System (Integration with QX)

To enable true, non-custodial asset management, MSVault integrates with the QX asset contract. The process is as follows:
* **Granting Rights:** Before depositing an asset into a vault, the owner must first grant management rights for that asset to the MSVault contract. This is a deliberate action that proves ownership and authorizes the contract to hold the tokens.
* **Depositing:** Once rights are granted, the user can deposit the assets into their vault.
* **Releasing:** When an asset is successfully released, MSVault transfers the asset to the destination and simultaneously returns management of that asset to the standard QX contract.
* **Revoking Rights:** Users retain ultimate control. They can revoke the management rights they granted to MSVault for any assets that have not yet been deposited into a vault.

### 4. New Functions for Asset Management

To support these new features, several new procedures and functions have been added:

* `depositAsset`: Deposits a specific amount of a custom asset into a vault.
* `releaseAssetTo`: Initiates a multi-signature release proposal for a custom asset.
* `resetAssetRelease`: Cancels an owner's approval for a pending asset release.
* `revokeAssetManagementRights`: Allows a user to reclaim management rights from the MSVault contract.
* `getVaultAssetBalances`: A read-only function to view all the different custom assets and their balances within a vault.
* `getAssetReleaseStatus`: A read-only function to check the approval status of a pending asset release.

## Fee Structure
The existing fee structure remains consistent and now applies to asset-related operations as well, ensuring fairness and continued revenue generation for shareholders.

* RELEASE_FEE = 100k: Now also applies when proposing an asset release (releaseAssetTo).
* RELEASE_RESET_FEE = 1mil: Now also applies when resetting an asset release proposal (resetAssetRelease).
* HOLDING_FEE, REGISTERING_FEE, and DEPOSIT_FEE remain unchanged.

Note: When an asset is successfully released, the underlying QX contract may charge a small fee (currently 100 QUs) for the management rights transfer, **which is paid from the vault's Qubic balance**. This fee is required by Qx themselves, not from MsVault.

## Shareholder Revenues

The revenue model is unchanged. All fees collected by the MSVault contract (for both Qubic and asset operations) are distributed to shareholders. There is no development fee. This upgrade increases the potential utility of the contract, which is expected to lead to higher fee generation and greater returns for shareholders.

# Technical implementation

From https://github.com/qubic/core/pull/485
