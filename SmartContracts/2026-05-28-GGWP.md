...
2. User calls Stake(amount) to lock shares
3. BEGIN_EPOCH: Rewards distributed proportionally
4. User calls RequestUnstake(amount) to initiate withdrawal
5. After 2-epoch delay: User calls FinalizeUnstake() to receive shares
6. User calls ClaimStakingRewards() to claim accumulated rewards
