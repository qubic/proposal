...
## Proposal
Allow Deployment of the new Quottery Smart Contract in EP209 on Qubic
...
* **Shareholder Governance (QTRYGOV):** Token holders control protocol parameters via voting.
    * Governed parameters include:
        ...
        * dispute deposit
        * fee per day
        * **Game Operator address**
    * Mechanism:
        * proposals submitted by holders
        * voting tracked on-chain
...
* **QTRY will migrate to QUSD (change code) without making another proposal**
* **After the deployment, QTRY will be allowed to add bug fixes (if any) without making another proposal**
...
|--------|-------------|-----------------|
| 1 | `BasicInfo` | Returns contract parameters, fees, and global statistics |
| 2 | `GetEventInfo` | Returns full details and state of a specific event |
| 3 | `GetOrders` | Returns current order book data for an event |
| 4 | `GetActiveEvent` | Returns a list of currently active events |
| 5 | `GetEventInfoBatch` | Returns metadata for multiple events in a single call |
| 6 | `GetUserPosition` | Returns user positions for a specific event |
| 7 | `GetApprovedAmount` | Returns approved token amount for a given user |
| 8 | `GetTopProposals` | Returns top governance proposals based on votes |

**Procedures (state-changing):**

| **ID** | **Procedure** | **Description** |
|--------|--------------|-----------------|
| 1 | `CreateEvent` | Creates a new prediction event with defined parameters |
| 2 | `AddToAskOrder` | Places a sell order and attempts to match it |
| 3 | `RemoveAskOrder` | Cancels a sell order and restores user position |
| 4 | `AddToBidOrder` | Places a buy order, locks funds, and triggers matching |
| 5 | `RemoveBidOrder` | Cancels a buy order and refunds locked funds |
| 6 | `PublishResult` | Publishes the initial result of an event (by operator) |
| 7 | `TryFinalizeEvent` | Finalizes an event after dispute window if no dispute is active |
| 8 | `Dispute` | Initiates a dispute on an event result with a deposit |
| 9 | `ResolveDispute` | Allows computors to vote and resolve disputed events |
| 10 | `UserClaimReward` | Allows a user to claim rewards from finalized events |
| 11 | `GOForceClaimReward` | Allows operator to batch-distribute rewards |
| 12 | `TransferQUSD` | Transfers QUSD(GARTH) tokens within contract context |
| 13 | `TransferShareManagementRights` | Transfers governance/management rights to another address |
| 14 | `CleanMemory` | Cleans finalized events and frees contract storage |
| 15 | `TransferQTRYGOV` | Transfers governance tokens between users |

### Source Code

PR: https://github.com/qubic/core/pull/819
